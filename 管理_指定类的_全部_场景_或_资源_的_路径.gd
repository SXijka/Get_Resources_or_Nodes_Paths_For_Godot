...
static func 异步_获取_指定目录下_指定类_及其派生类_文件的_路径(基类: String, 是_场景: bool = true, 根目录: String = "res://") -> Array[String]:
	var 结果: Array[String] = []
	var 文件列表: Array[String] = []
...
static func _扫描目录(当前路径: String, 文件列表: Array[String]):
	var dir = DirAccess.open(当前路径)
	if not dir:
		push_error("执行_扫描目录 失败: 无法打开目录: %s" % 当前路径)
		return
		
	dir.list_dir_begin()
	var 文件名 = dir.get_next()
	while 文件名 != "":
		if 文件名 != "." and 文件名 != "..":
			var 完整路径 := 当前路径.path_join(文件名)
